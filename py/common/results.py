...
class ScanResults:
    ...
    def __enter__(self):
        ...
        def signal_handler(signum, frame):
            ...
            if self.subproc is not None:
                # forward the signal to the process group of the subtask being executed
                try:
                    pg = os.getpgid(self.subproc.pid)
                    os.killpg(pg, signum)
                except Exception as e:
                    self.error("failed to kill process group: %s" % e)
    ...
    def exec_cmd(self, cmd, shell=False, echo=True):
        ...
        try:
            self.subproc = subprocess.Popen(
                cmd, stdout=self.log_fd, stderr=self.log_fd, shell=shell, preexec_fn=os.setsid)
    ...
    def get_cmd_output(self, cmd, shell=True):
        self.handle_ec()
        self.subproc = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=self.log_fd, shell=shell, preexec_fn=os.setsid)
